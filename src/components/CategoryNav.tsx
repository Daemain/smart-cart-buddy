...
const CategoryNav: React.FC<CategoryNavProps> = ({
  ...
  return <div className="relative">
      ...
      <div ref={scrollContainerRef} onScroll={handleScroll} className="flex overflow-x-auto no-scrollbar pb-1 px-[8px]">
        {categories.map(category => <button key={category.id} className={cn("flex items-center gap-1 px-3 py-2 text-sm font-medium rounded-full transition-colors mr-2 whitespace-nowrap", activeCategory === category.id ? "bg-primary text-primary-foreground" : "bg-muted hover:bg-muted/80 text-muted-foreground")} onClick={() => setActiveCategory(category.id)}>
            {category.icon}
            <span>{category.label}</span>
            <span className="inline-flex items-center justify-center rounded-full bg-primary-foreground/20 px-1.5 text-xs min-w-5">
              {counts[category.id]}
            </span>
            {category.premium && !isPremium && <Lock className="h-3 w-3 ml-0.5 text-muted-foreground" />}
          </button>)}
      </div>
