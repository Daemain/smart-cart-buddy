...
const GroceryItem: React.FC<GroceryItemProps> = ({
  ...
  return <div className={cn("grocery-item animate-fade-in rounded-lg shadow-sm border flex flex-col", isCompleted ? "bg-app-light-green border-app-light-green/80" : "bg-white border-border", isFrequent && !isCompleted ? "bg-app-light-blue border-app-light-blue/80" : "")}>
      <div className="flex items-left px-3 pt-3 pb-2 gap-3">
        <Checkbox id={`check-${id}`} checked={isCompleted} onCheckedChange={() => toggleCompletion(id)} className="h-5 w-5 flex-shrink-0" />
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-between gap-2">
            ...
            {quantity && <div className="text-sm font-regular bg-primary/10 text-primary px-2 py-0.5 rounded-full flex-shrink-0">
                {quantity}
              </div>}
          </div>
        </div>
      ...
      {notes && <div className="px-3 pb-3 pt-1 w-full">
          {isMobile || notes.length <= 120 ? <div className="text-sm text-muted-foreground bg-muted/20 p-2 rounded-md">
              {notes}
            </div> : <div className="relative">
              <div className="text-sm text-muted-foreground bg-muted/20 p-2 rounded-md line-clamp-2">
                {notes}
              </div>
