...
const GroceryItem: React.FC<GroceryItemProps> = ({
  ...
  return (
    <div className={cn(
      "border rounded-lg overflow-hidden mb-3 shadow-sm",
      isCompleted ? "border-muted bg-muted/20" : "border-border",
      isFrequent ? "border-primary/40" : ""
    )}>
      ...
      {notes && (
        <div className="px-3 pb-3 w-full">
          {isMobile || notes.length <= 120 ? (
            <div className="text-sm text-muted-foreground bg-muted/20 p-2 rounded-md w-full border border-border/50">
              {notes}
            </div>
          ) : (
                ...
                <Tooltip>
                  <TooltipTrigger asChild>
                    <Button 
                      variant="secondary" 
                      size="sm" 
                      className="mt-1 h-6 text-xs w-auto"
                    >
