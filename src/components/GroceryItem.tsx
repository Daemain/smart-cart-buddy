...
const GroceryItem: React.FC<GroceryItemProps> = ({
  ...
  return <div className={cn("grocery-item animate-fade-in rounded-lg shadow-sm border", isCompleted ? "bg-app-light-green border-app-light-green/80" : "bg-white border-border", isFrequent && !isCompleted ? "bg-app-light-blue border-app-light-blue/80" : "")}>
      <Checkbox id={`check-${id}`} checked={isCompleted} onCheckedChange={() => toggleCompletion(id)} className="h-5 w-5" />
      ...
      <div className="flex-1 min-w-0">
        ...
        {notes && <div className="relative">
            <div className="text-sm text-muted-foreground mt-0.5 line-clamp-1 pr-5">
              {notes}
            </div>
            
            {notes.length > 60 && <TooltipProvider>
                <Tooltip>
                  <TooltipTrigger asChild>
                    <Button variant="ghost" size="icon" className="h-5 w-5 absolute right-0 top-0 text-muted-foreground hover:text-foreground">
